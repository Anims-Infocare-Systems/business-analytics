...
This document provides the technical leadership, backend team, frontend team, and DevOps engineers with a complete architectural analysis, **End-to-End Process Diagrams (EPD)**, codebase bottleneck audit, and execution roadmap to scale the **Business Analytics** platform to **5,000 – 8,000 users** on **BigRock VPS (AlmaLinux 9)**.
...
* **Location:** `DashboardLayout.jsx` (lines 1034-1041) & `views.py` (lines 615-640)
...
* **The Problem:** Each heartbeat executes:
  ...
  3. Session write-back because `SESSION_SAVE_EVERY_REQUEST = True` in `settings.py` (line 146).
...
* **Location:** `views.py` (lines 648-675)
...
* **Location:**
  * `SalesAnalysis.jsx` (lines 4604-4825): Fires **14 simultaneous HTTP requests** upon load (`p1`, `pGT`, `p2` ... `p13`).
  * `PurchaseAnalysis.jsx` (lines 3750-4084): Fires **10+ simultaneous HTTP requests**.
  * `views_plantperformance.py` (lines 4855-4916): Single bundle endpoint launches a Python `ThreadPoolExecutor(max_workers=8)` running up to 30 SQL sub-queries.
...
* **Location:** `accounts/utils/db.py` (lines 14-31)
* **The Mechanism:**
  ```python
...
* **Location:** `backend/settings.py` (line 84)
