        ...
        config: {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: [
                        "#3b82f6","#06b6d4","#10b981","#f97316","#8b5cf6","#ec4899",
                        "#f59e0b","#ef4444","#6366f1","#84cc16","#14b8a6","#f43f5e"
                    ],
                }],
            },
            options: {
...
// ─── FIX: format date using LOCAL timezone (avoids UTC day-shift) ──
                    ...
                    if (def.id === "sales-1") {
                        chartRef.current = new Chart(canvasRef.current, {
                            ...
                            data: {
                                labels: data.labels || [],
                                datasets: [
                                    { label: "PO Value (₹L)", data: data.po || [], backgroundColor: "#3b82f6", borderRadius: 4 },
                                    { label: "Sales Value (₹L)", data: data.sales || [], backgroundColor: "#10b981", borderRadius: 4 },
                                ],
                            },
                            options: { ...def.config.options },
                        });
                    } else if (def.id === "quality-1") {
                        // Customer Complaint Distribution – Pie chart monthwise
                        chartRef.current = new Chart(canvasRef.current, {
                            type: "pie",
                            data: {
                                labels: data.labels || [],
...
// ─── Preview Modal ────────────────────────────────────────────
function PreviewModal({ def, onClose, initialDateRange }) {
    const [modalDateRange, setModalDateRange] = useState(
        initialDateRange?.from ? initialDateRange : { from: null, to: null }
    );
                    ...
                    if (def.id === "sales-1") {
                        chartRef.current = new Chart(canvasRef.current, {
                            ...
                            data: {
                                labels: data.labels || [],
                                datasets: [
                                    { label: `PO Value (₹L) — ${data.fy || ""}`, data: data.po || [], backgroundColor: "#3b82f6", borderRadius: 4 },
                                    { label: `Sales Value (₹L) — ${data.fy || ""}`, data: data.sales || [], backgroundColor: "#10b981", borderRadius: 4 },
                                ],
                            },
                            options: {
                                ...def.config.options,
                                plugins: {
                                    ...
                                    title: { display: true, text: `${data.company || ""} · ${data.fy || ""}  (${data.from} → ${data.to})`, font: { size: 10 }, color: "#64748b", padding: { bottom: 8 } },
                                },
                            },
                        });
                    } else if (def.id === "quality-1") {
...
export default function Charts() {
    ...
    const handlePreview = (def) => setPreview({ def, dateRange });
