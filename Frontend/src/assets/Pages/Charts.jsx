    ...
    if (def.id === "sales-1" || def.id === "sales-2" || def.id === "quality-1" || def.id === "quality-2" || def.id === "quality-3" || def.id === "quality-4" || def.id === "production-1" || def.id === "production-2" || def.id === "production-3" || def.id === "production-4" || def.id === "operations-1" || def.id === "operations-2" || def.id === "purchase-1" || def.id === "purchase-2" || def.id === "vendor-2") {
      ...
      fetch(url, { credentials: "include", signal: ac.signal })
        ...
        .then(data => {
          ...
          else if (def.id === "quality-2") chartRef.current = new Chart(canvasRef.current, { type: "line", data: { labels: data.labels || [], datasets: [{ label: `Rejections — ${data.fy || ""}`, data: data.data || [], borderColor: "#ec4899", backgroundColor: "rgba(236,72,153,0.1)", tension: 0.4, fill: true, pointRadius: 3 }] }, options: { ...def.config.options, plugins: { ...def.config.options.plugins, title: { display: true, text: `Rejection Trend ${data.fy || ""} (${data.from} → ${data.to})`, font: { size: 10 }, color: "#64748b", padding: { bottom: 8 } } } } });
          else if (def.id === "quality-3") chartRef.current = new Chart(canvasRef.current, { type: "bar", data: { labels: data.labels || [], datasets: [{ label: `Rework Items — ${data.fy || ""}`, data: data.data || [], backgroundColor: def.config.data.datasets[0].backgroundColor, borderRadius: 4 }] }, options: { ...def.config.options, plugins: { ...def.config.options.plugins, title: { display: true, text: `Rework Trend ${data.fy || ""} (${data.from} → ${data.to})`, font: { size: 10 }, color: "#64748b", padding: { bottom: 8 } } } } });
          else if (def.id === "quality-4") chartRef.current = new Chart(canvasRef.current, { type: "line", data: { labels: data.labels || [], datasets: [{ label: `Actual PPM — ${data.fy || ""}`, data: data.data || [], borderColor: "#f97316", backgroundColor: "rgba(249,115,22,0.1)", tension: 0.4, fill: true, pointRadius: 3 }] }, options: { ...def.config.options, plugins: { ...def.config.options.plugins, title: { display: true, text: `Internal Mac Rejection PPM ${data.fy || ""} (${data.from} → ${data.to})`, font: { size: 10 }, color: "#64748b", padding: { bottom: 8 } } }, scales: { ...def.config.options.scales, y: { ticks: { font: { size: 9 }, callback: val => val.toLocaleString() + ' PPM' } } } } });
          else if (def.id === "production-1") chartRef.current = new Chart(canvasRef.current, { type: "line", data: { labels: data.labels || [], datasets: [{ label: `${modalOperator || 'Operator'} Efficiency % — ${data.fy || ""}`, data: data.data || [], borderColor: "#3b82f6", backgroundColor: "rgba(59,130,246,0.1)", tension: 0.4, fill: true, pointRadius: 3 }] }, options: { ...def.config.options, plugins: { ...def.config.options.plugins, title: { display: true, text: `Operator Efficiency: ${modalOperator || ''} ${data.fy || ""} (${data.from} → ${data.to})`, font: { size: 10 }, color: "#64748b", padding: { bottom: 8 } } }, scales: { ...def.config.options.scales, y: { min: 0, max: 100, ticks: { font: { size: 9 }, callback: val => val + '%' } } } } });
          else if (def.id === "production-2") chartRef.current = new Chart(canvasRef.current, { type: "bar", data: { labels: data.labels || [], datasets: [{ label: `Overall Efficiency % — ${data.fy || ""}`, data: data.data || [], backgroundColor: "#06b6d4", borderRadius: 4 }] }, options: { ...def.config.options, plugins: { ...def.config.options.plugins, title: { display: true, text: `Overall Operator Efficiency ${data.fy || ""} (${data.from} → ${data.to})`, font: { size: 10 }, color: "#64748b", padding: { bottom: 8 } } }, scales: { ...def.config.options.scales, y: { min: 0, max: 100, ticks: { font: { size: 9 }, callback: val => val + '%' } } } } });
          else if (def.id === "production-3") {
            const palette = ["#3b82f6","#06b6d4","#10b981","#f97316","#8b5cf6","#ec4899","#f43f5e","#84cc16","#14b8a6","#6366f1","#f59e0b","#a855f7"];
  ...
  return createPortal(
    <div className={`ch-modal${closing ? " ch-modal--closing" : ""}`} onMouseDown={e => { if (e.target === e.currentTarget) animatedClose(); }}>
      <div className="ch-modal__content" style={{ backgroundColor: "#ffffff" }}>
        ...
        <div className="ch-modal__canvas-wrap" style={{ backgroundColor: "#ffffff" }}><canvas ref={canvasRef} /></div>
